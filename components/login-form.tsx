...
export function LoginForm({
  ...
  const subheading = isSignup
    ? "Start building your next great app in seconds."
    : "Sign in to your Fast NextJs app by Tubeguruji"
  const buttonLabel = isSignup ? "Sign up with Google" : "Login with Google"
        ...
        <CardContent className="grid p-0 md:grid-cols-2 h-[500px]">
          ...
          <div className="relative flex flex-col items-center justify-center p-8 text-center gap-6 bg-white">
            ...
            <div className="relative z-10">
              <h1 className="text-3xl font-extrabold text-blue-600">{heading}</h1>
              <p className="text-muted-foreground mt-2">
                {subheading.split("Fast NextJs")[0]}
                <span className="font-semibold text-pink-600">Fast NextJs</span>
                {subheading.split("Fast NextJs")[1]}
              </p>
            </div>

            <div className="relative z-10 w-full space-y-3">
              <Authentication redirectTo="/dashboard">
              <Button
                variant="outline"
              ...
              >
                {switchText} {isSignup ? "Sign in" : "Sign up"}
              </Link>
            </div>
          </div>
